...
Trains one model per (coherence weight, seed) pair and records accuracy and
coherence quality on the held-out test set. Results are saved both per-seed
and as mean ± std averaged over all seeds. Supports three coherence scoring
modes: sequential, prefix_mean, and combined.

Usage:
    python scripts/train.py --max_train_playlists 128 --num_epochs 5

Full sweep (10 seeds, default lambda grid):
...
def _average_runs(seed_runs: list[dict]) -> dict:
    """Mean and std of test metrics across a list of per-seed run results."""
    def _avg(dicts):
        keys = dicts[0].keys()
        n = len(dicts)
        mean = {k: sum(d[k] for d in dicts) / n for k in keys}
        std = {k: (sum((d[k] - mean[k]) ** 2 for d in dicts) / n) ** 0.5 for k in keys}
        return mean, std

    best_mean, best_std = _avg([r['test_best'] for r in seed_runs])
    final_mean, final_std = _avg([r['test_final'] for r in seed_runs])
    bvn = sum(r['best_val_ndcg'] for r in seed_runs) / len(seed_runs)

    return {
        'mean': {'test_best': best_mean, 'test_final': final_mean, 'best_val_ndcg': bvn},
        'std': {'test_best': best_std, 'test_final': final_std},
    }
...
def run_single(
    ...
    best_val_ndcg = -1.0
    ckpt_path = output_dir / f'ckpt_{args.coherence_mode}_lambda_{coherence_weight:.4f}_seed_{seed}.pt'
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument('--max_train_playlists', type=int, default=8000)
    ...
    p.add_argument('--seed', type=int, default=10,
                   help='seed for data loading and train/val/test splits')
    p.add_argument('--seeds', type=int, nargs='+',
                   default=list(range(10)),
                   help='training seeds to sweep; one model trained per (lambda, seed)')

    p.add_argument(
        '--coherence_weights', type=float, nargs='+',
        default=[0.0, 0.05, 0.1, 0.2, 0.5, 1.0, 2.0, 5.0, 10.0, 15.0, 20.0, 25.0, 50.0],
    )
...
def main() -> None:
    ...
    n_lam = len(args.coherence_weights)
    n_seeds = len(args.seeds)
    print(f'\nsweeping {n_lam} lambda values x {n_seeds} seeds (mode: {args.coherence_mode})')
    ...
    def _row(run: dict, subkey: str) -> str:
        lam = run['coherence_weight']
        mean = run['mean'][subkey]
        ndcg = mean.get(f'test_ndcg_{primary_k}', float('nan'))
        coh = mean.get('test_eval_coherence', float('nan'))
        ce = mean.get('test_ce_loss', float('nan'))
        all_norms = [e['train_grad_norm'] for sr in run['seed_runs'] for e in sr['epochs']]
        med_gn = sorted(all_norms)[len(all_norms) // 2]
        return (f'{lam:>8.3f}  {ndcg:>10.4f}  {coh:>10.4f}'
                f'  {ce:>10.4f}  {med_gn:>14.3f}')

    print(f'\npareto frontier - final epoch (mean over {n_s} seeds):')
    print(header)
    print(sep)
    for run in all_runs:
        print(_row(run, 'test_final'))

    print(f'\npareto frontier - best val nDCG@{primary_k} checkpoint (mean over {n_s} seeds):')
    print(header)
    print(sep)
    for run in all_runs:
